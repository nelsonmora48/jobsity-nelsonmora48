...
module.exports.bootstrap = async function () {
  ...
  // Create Exchange
  global.amqp_channel.assertExchange('chat_to_bot', 'fanout', {
    durable: false,
  });
  global.amqp_channel.assertExchange('bot_to_chat', 'fanout', {
    durable: false,
  });
  // Create Queue
  const q_to_bot = await global.amqp_channel.assertQueue('', {});
  const q_to_chat = await global.amqp_channel.assertQueue('', {});
  ...
  await global.amqp_channel.bindQueue(q_to_bot.queue, 'chat_to_bot', '');
  await global.amqp_channel.bindQueue(q_to_chat.queue, 'bot_to_chat', '');

  let stock_response = {};
  await global.amqp_channel.consume(q_to_chat.queue, (msg) => {
    stock_response = JSON.parse(msg.content.toString());
    sails.sockets.broadcast(stock_response.user, 'message', [
      { user: 'Stock Bot', payload: stock_response.response },
    ]);
  });
